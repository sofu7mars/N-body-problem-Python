...
def n_body_diffy1(t, y):
    ...
    velocities = y[3*n:].reshape((n, 3))
    accelerations = np.zeros_like(positions)
    for i in range(len(universe.bodies)):
        for j in range(len(universe.bodies)):
            if i != j:
                rji = positions[i] - positions[j]
                distance = np.linalg.norm(rji)
                accelerations[i] += -G * universe.bodies[j].mass *rji / distance**3 
